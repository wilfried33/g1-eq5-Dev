...
|     | ID01  | Dans le fichier "routes/projects.js", créer la route POST "/projects" qui reçoivent les données d'un projet (name et key) et ajoute le projet en question à la base de donnée via le service projectService. Si il existe déjà un projet avec le même nom ou la même key, le retour sera 400.                                                               | | 0.5     |
|     | ID01  | Implémenter le test de la route POST "/projects" en envoyant une requête qui vérifie que le code de retour est bon (200). Le test s'assurera également que si un élément contenant la même key ou le même nom est ajouté, le code de retour soit 400.                        | |   0.5     |
|     | ID01  | Créer le fichier addProject.ejs contenant un formulaire composé des champs "nom" et "key". Le formulaire envoie une requête POST avec ces deux champs à l'url "/projects".                                                                                                  | | 1      |
|     | ID01  | Dans le fichier projectService.js, ajouter le fonction updateProject(name, key) permettant de modifier le titre du projet de key "key" via mongoDB.  | |    0.5    |
|     | ID01  | Implémenter le tests de la fonction updateProject du fichier projectService.js, qui vérifie qu'elle modifie un projet en bd.                         | |    0.5    |
|     | ID01  | Dans le fichier "routes/projects.js", créer la route PUT "/projects" qui reçoit les données d'un projet (name et key) et modifient le projet en question à la base de donnée via le service projectService. Le code 400 sera retourné si l'US à modifier n'existe pas.                                                                 | |    0.5    |
|     | ID01  | Implémenter le test de la route PUT "/projects" en envoyant une requête qui vérifie que le code de retour est bon (200). Le test s'assurera également que si l'US à modifier n'existe pas, le code de retour soit 400.                         | |    0.5    |
|     | ID01  | Dans le fichier project.ejs, ajouter un bouton "Modifier" pour chaque projet qui afficher un formulaire composé des champs, avec valeur "nom" et "key". Le formulaire envoie une requête PUT avec ces deux champs à l'url "/projects".                                      | | 1      |
|     | ID01  | Ecrire le scénario du test e2e en Gherkin dans le fichier "test/e2e/ID01.feature"             | | 0.25   |
|     | ID01  | Implémenter le test E2E de l'US décrit dans le fichier "test/e2e/ID01.feature"                                                                                                              | | 1.5    |
|     | ID30  | Créer dans le fichier projectService.js, la fonction getProjectList() qui renvoie la liste des projets présents en bd.                               | |    0.5    |
|     | ID30  | Dans le fichier "routes/projects.js", créer la route GET "/projects" qui renvoie la liste des projets via le projectService.                         | |    0.5    |
|     | ID30  | Implémenter les tests de la route "/projects" en envoyant une requête GET qui vérifie que le retour est bien un projet existant.                 | |    0.5    |
|     | ID30  | Créer le fichier projects.ejs permettant d'afficher les projets renvoyés en GET. Chaque projet est sélectionnable afin d'aller respectivement sur sa page du backlog.                                                                                                       | |    1    |
|     | ID30  | Ecrire le scénario du test e2e en Gherkin dans le fichier "test/e2e/ID30.feature"             | |    0.25    |
|     | ID30  | Implémenter le test E2E de l'US décrit dans le fichier "test/e2e/ID30.feature"                                                                                                                    | |   1    |
|     | ID28  | Créer le fichier header.ejs contenant la bar de navigation, avec des liens vers les pages "Projets", "Backlog", "Kanban", "Planning", "Releases" et "Tests".                                                                                                                | |   1.5    |
|     | ID28  | Ecrire le scénario du test e2e en Gherkin dans le fichier "test/e2e/ID28.feature"             | |    0.25    |
|     | ID28  | Implémenter le test E2E de l'US décrit dans le fichier "test/e2e/ID28.feature"                                                                                                                     | |    1.5    |
|     | ID02  | Créer le fichier backlogService.js contenant la fonction addUserStory(project, name, description) permettant d'ajouter une US en bd. Cette fonction génère un id unique et l'attribut à l'US.                                                                               | |    0.5    |
|     | ID02  | Implémenter le test de la fonction addUserStory du fichier backlogService.js, qui vérifie qu'elle ajoute bien une US en bd.                          | |    0.5    |
|     | ID02  | Dans le fichier "routes/backlog.js", créer la route GET "/userStories" qui renvoie le formulaire du fichier addUserStory.ejs                         | |    1    |
|     | ID02  | Dans le fichier "routes/backlog.js", créer la route POST "/userStories" qui reçoit les données d'une US (project, name et description) et ajoute l'US en question à la bd via le service backlogService.                                                                    | |    0.5    |
|     | ID02  | Implémenter le test de la route "/userStories" en envoyant une requête POST qui vérifie que le code de retour est bon (200).                     | |    0.5    |
|     | ID02  | Créer le fichier addUserStory.ejs contenant un formulaire composé des champs "name", "description" et "project". Le formulaire envoie une requête POST avec ces deux champs à l'url "/userStories".                                                                         | |    1    |
|     | ID02  | Ecrire le scénario du test e2e en Gherkin dans le fichier "test/e2e/ID02.feature"             | |    0.25    |
|     | ID02  | Implémenter le test E2E de l'US décrit dans le fichier "test/e2e/ID02.feature"                                                                                                                 | |    1.5    |
...
|     | ID03  | Implémenter le test de la fonction updateUserStory qui verifie que la modification de l'US a bien lieu en BD dans le cas où aucune tâche n'est liée à cette US                                                                                                              | |        |
|     | ID03  | Implémenter le test de la fonction updateUserStory qui verifie que la fonction envoie une erreur si une tâche est associée à cette US.               | |        |
...
|     | ID03  | Implémenter le test de la route "/userStories" en envoyant une requête PUT qui vérifie que le code de retour est bon (200).                      | |        |
...
|     | ID03  | Implémenter le test de la fonction deleteUserStory qui verifie que la modification de l'US a bien lieu en BD dans le cas où aucune tâche n'est liée à cette US                                                                                                              | |        |
|     | ID03  | Implémenter le test de la fonction deleteUserStory qui verifie que la fonction envoie une erreur si une tâche est associée à cette US.               | |        |
...
|     | ID03  | Implémenter le test de la route "/userStories/:id" en envoyant une requête DELETE qui vérifie que le code de retour est bon (200).               | |        |
|     | ID03  | TODO ejs                      | |        |
...
|     | ID05  | Implémenter le test de la fonction setUserStorySprint qui verifie que l'attribution du sprint d'une US a bien lieu en BD lorsque l'US ou le sprint existent.                                                                                                                | |        |
|     | ID05  | Implémenter les tests de la fonction setUserStorySprint qui verifient qu'une erreur est renvoyée si le sprint (resp. l'US) n'existe pas.             | |        |
...
|     | ID07  | Implémenter le test de la fonction updateSprint qui verifie que la modification du sprint a bien lieu en BD.                                         | |        |
...
|     | ID07  | Implémenter le test de la fonction deleteSprint qui verifie que la suppression du sprint a bien lieu en BD.                                          | |        |
...
|     | ID09  | Implémenter les tests de la route "/backlog" en envoyant une requête GET qui vérifie que le retour est bien le backlog existant.                 | |   0.5  |
...
|     | ID10  | Implémenter les tests de la route "/userStories/:id" en envoyant une requête GET avec un id existant qui vérifie que le retour est bien l'US concernée. Le test vérifie également que si l'id est eronné, l'erreur 400 est renvoyée.                                    | |   0.5  |
...
|     | ID11  | Dans le fichier "routes/tasks.js", créer la route GET "/tasks/create" qui renvoie le formulaire du fichier addTask.ejs                               | Dans le fichier "routes/tasks.js", créer la route GET "/tasks/create" qui renvoie le formulaire du fichier addTask.ejs |  0.5   |
...
|     | ID11  | Implémenter le test de la route "/tasks" en envoyant une requête POST qui vérifie que le code de retour est bon (200).                           | |  0.5   |
...
|     | ID11  | Implémenter le test de la fonction updateTask qui verifie que la modification de la tache a bien lieu en BD.                                         | |  0.5   |
...
|     | ID11  | Implémenter le test de la route "/tasks" en envoyant une requête PUT qui vérifie que le code de retour est bon (200).                            | |  0.5   |
...
|     | ID45  | Implémenter les tests de la route "/tasks" en envoyant une requête GET qui vérifie que le retour est bien la liste des tâches existant.          | |  0.5   |
...
|     | ID37  | Implémenter le test de la fonction getUSByDifficulty qui verifie que la fonction renvoie bien les US par difficulté.                                               | |     |
...
|     | ID33  | Implémenter le test de la fonction updateDOD qui verifie que la modification du DOD a bien lieu en BD.                                               | |  0.5   |
