...
|  TDES-04  | ALL   | Créer le Dockerfile ainsi que le docker-compose.yml permettant de build l'application depuis n'importe quelle machine | / |   2   |
|  TDES-05  | ALL   | Créer le linter dans le fichier .eslintrc.json | / |  1  |
...
|  TTES-03  | ID01  | Implémenter le test de la route POST "/projects" en envoyant une requête qui vérifie que le code de retour est bon (200). Le test s'assurera également que si un élément contenant la même key ou le même nom est ajouté, le code de retour soit 400.                        | |   0.5     |
...
|  TTES-05  | ID01  | Implémenter le test de la route PUT "/projects" en envoyant une requête qui vérifie que le code de retour est bon (200). Le test s'assurera également que si l'US à modifier n'existe pas, le code de retour soit 400.                         | |    0.5    |
|  TDEV-07  | ID01  | Dans le fichier project.ejs, ajouter un bouton "Modifier" pour chaque projet qui afficher un formulaire composé des champs, avec valeur "nom" et "key". Le formulaire envoie une requête PUT avec ces deux champs à l'url "/projects".                                      | | 1      |
