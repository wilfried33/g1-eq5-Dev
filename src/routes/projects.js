...
router.get('/select', (req, res) => {
    ...
    res.status(200).json({valid:'Projet bien sélectionné'});
});
...
router.post('/', (req, res) => {
    ...
    if (!name || !key)
        return res.status(400).render('addProject', {error:'Champ manquant'});
...
function renderProjectList(status, req, res, error){
    projectService.getProjectList()
        ...
        .catch(() => res.status(400).render('projects', {projects: [], error:'Impossible de charger la liste des projets'}));
}
